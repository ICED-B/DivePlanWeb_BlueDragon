...
import { BarChart2, Waves, Clock, ArrowDown, MapPin, Flame } from 'lucide-react';
...
interface DeepDive { id: number; start_time?: string; max_depth_m?: number; duration_min?: number; site?: string; diver_name?: string; }
interface GlobalOverview { basic?: BasicStats; top_sites?: TopSite[]; top_deep_dives?: DeepDive[]; }
    ...
    <div className="divide-y" style={{ borderColor: 'var(--border-muted)' }}>
      {rows.map((r, i) => (
        <div key={i} className="flex items-center justify-between py-2 text-sm">
          ...
          <span className="font-semibold" style={{ color: 'var(--accent)' }}>{r.value}</span>
        </div>
      ))}
    </div>
  );
...
export default function GlobalStatsPage() {
  ...
  useEffect(() => {
    ...
    const endpoints: Record<TabId, string> = {
      ...
      gas:       '/stats/global/gas-mix',
      tanks:     '/stats/global/tanks',
      tags:      '/stats/global/tags',
      events:    '/stats/global/events',
      circuits:  '/stats/global/circuits',
      licenses:  '/stats/global/licenses',
      countries: '/stats/global/countries',
      regions:   '/stats/global/regions',
    };
    ...
    <PageContainer>
      <div className="mb-6">
        <h1 className="text-3xl font-bold flex items-center gap-3" style={{ color: 'var(--text)' }}>
          <BarChart2 style={{ color: 'var(--accent)' }} size={28} />
          {t('stats.global_title')}
        </h1>
        <p className="mt-1 text-sm" style={{ color: 'var(--text-muted)' }}>
          {t('stats.global_subtitle')}
        </p>
      </div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <RankTable rows={overview.top_deep_dives.map((d) => ({
                        label: `#${d.id}${d.diver_name ? ` — ${d.diver_name}` : ''}`,
